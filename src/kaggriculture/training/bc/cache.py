...
def _path(source: Path, directory: Path, rules: CacheRules, players) -> Path:
    stat = source.stat()
    identity = (str(source.resolve()), stat.st_size, stat.st_mtime_ns, rules, players, _VERSION)
    return directory / f"{hashlib.sha256(repr(identity).encode()).hexdigest()}.npz"


def prepare_episode(source: Path, directory: Path, rules: CacheRules, players=None) -> Path | None:
    """1 episodeを固定shape shardへatomicに保存する。"""
    destination = _path(source, directory, rules, players)
    if destination.exists():
        return destination
    stats = BuildStats()
    selected = (source, players) if players is not None else source
    samples = list(iter_samples([selected], rules, stats))
...
def prepare_episodes(sources, directory: Path, rules: CacheRules):
    """episode群を変換し、利用可能なshard pathを返す。"""
    paths = []
    for item in sources:
        source, players = item if isinstance(item, tuple) else (item, None)
        result = prepare_episode(Path(source), directory, rules, players)
...
def load_shard(path: Path) -> BCBatch:
    with np.load(path, allow_pickle=False) as data:
        ...
        return BCBatch(states, data["players"], intent, data["slot_mask"])
...
def iter_batches(paths, batch_size: int, *, seed: int, shuffle: bool, drop_last: bool):
