...
先頭の軸は常に [バッチB, プレイヤー2, ...] の順(プレイヤー軸が無いのは
market/town/stepなど両プレイヤー共有のフィールドのみ)。
...
class State(NamedTuple):
    ...
    # ============================================================
    # ユニット位置 — shape [B, 2, 2](末尾はx,y)、hands系は [B, 2, MAX_HANDS, 2]
    # ============================================================
    farmer_pos: jnp.ndarray
    hands_pos: jnp.ndarray
    # そのhandスロットが今日実際に雇われているか(shape [B, 2, MAX_HANDS])
    hands_active: jnp.ndarray
    # 今日すでに何人雇ったか(次のHIREのコスト計算に使う)。shape [B, 2]
    hires_today: jnp.ndarray

    # ============================================================
    # 農場のその他の状態
    # ============================================================
    money: jnp.ndarray  # shape [B, 2]
    # 解放済み区画(constants.QUADRANTS順のbool)。shape [B, 2, N_QUADRANTS]
    ...
    # ============================================================
    # 納屋の在庫。shape [B, 2, N_SHED_ITEMS]
    shed: jnp.ndarray
    # 未使用の種の数。shape [B, 2, N_CROPS]
    seeds: jnp.ndarray
    # 主農夫が今日集めた持ち物。shape [B, 2, N_SHED_ITEMS]
    farmer_inventory: jnp.ndarray
    # 各handが今日集めた持ち物。shape [B, 2, MAX_HANDS, N_SHED_ITEMS]
    hands_inventory: jnp.ndarray

    # ============================================================
    # 市場(両プレイヤー共有、プレイヤー軸なし)
    # ============================================================
    # 品目ごとの在庫量。shape [B, N_PRODUCTS]
    ...
    # 街(両プレイヤー共有)
    # ============================================================
    # 出現済みの店を「種類ごとの出現数」で表す。shape [B, N_SHOPS]
    ...
    step: jnp.ndarray  # shape [B]
    # 乱数キー。weedの発生・街の店の抽選に使う。shape [B, 2](jax.random.PRNGKeyの形)
    rng_key: jnp.ndarray
