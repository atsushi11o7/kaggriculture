...
- **`MAX_HANDS`/`MAX_MARKET_ORDERS`**: 元のルールに存在しない、GPU移植版固有の上限([rules/constants.py](../rules/constants.py)参照)。配列のshapeを固定するために導入した
- **未対応の設定**: `marketParams`(品目ごとの価格曲線の上書き)は未対応。市場価格パラメータは[rules/game_params.py](../rules/game_params.py)の固定値のみ
...
| [rules/constants.py](../rules/constants.py) | policyと共有する定数・インデックス割り当て |
| [rules/game_params.py](../rules/game_params.py) | policyと共有するゲームバランスの数値パラメータ |
