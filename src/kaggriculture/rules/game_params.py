...
そのまま `jnp.array(...)` してテンソル演算に使える(jnp版はgame_params_jax.py参照)。

このモジュール自体はjaxに依存しない(policy/推論経路が意図せずjax[cuda12]を
importしてしまわないようにするため)。
...
# jnp版のCROP_IS_ONGOING等はgame_params_jax.pyへ分離した(policy/推論経路が
# jax[cuda12]一式をimportしてしまうのを避けるため。game_params_jax.py参照)。
